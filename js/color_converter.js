...
var ColorConverter = {
	...
	_RGBtoHSV : function  (RGB) {
		...
		result.h = Math.round(result.h * 360.0);
		result.s = Math.round(result.s * 100.0);
		result.v = Math.round(result.v * 100.0);
	...
	_HSVtoRGB : function  (HSV) {
		...
		if (s == 0) {
			...
			result.r = var_r * 255.0;
			result.g = var_g * 255.0;
			result.b = var_b * 255.0;
		}
	...
	_RGBtoCMYK : function (RGB){
		...
		result.c = Math.round( result.c * 100.0 );
		result.m = Math.round( result.m * 100.0 );
		result.y = Math.round( result.y * 100.0 );
		result.k = Math.round( result.k * 100.0 );
